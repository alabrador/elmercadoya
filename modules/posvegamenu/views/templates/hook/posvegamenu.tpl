...
<div class="pos-menu-vertical" data-more-less="{$menu_config.posvegamenu_more_less}">
	...
	<ul class="menu-content"> 
		{foreach from=$menus item=menu name=menus}	 			
				<li class="menu-item menu-item{$menu.id_posvegamenu_item} {$menu.item_class|escape:'html':'UTF-8'} {if ($menu.submenu_type == 0 && count($menu.sub_menu) > 0) ||($menu.submenu_type == 1 && !empty($menu.sub_menu))}hasChild{/if} {if isset($menu.selected_item) && $menu.selected_item == 1}active{/if}">
					...
					<a href="{if $menu.type_link==0}{$menu.link|escape:'html':'UTF-8'}{elseif $menu.type_link ==1}{$menu.custom_link|escape:'html':'UTF-8'}{else}javascript:void(0){/if}" {if $menu.new_window == 1} target="_blank" {/if}>
						...
						{if ($menu.submenu_type == 0 && count($menu.sub_menu) > 0) ||($menu.submenu_type == 1 && !empty($menu.sub_menu))} <i class="hidden-md-down icon_sub ion-ios-arrow-right"></i>{/if}
					</a>
